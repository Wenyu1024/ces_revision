...
tmp %>% 
  ...
  ylab("Gene expression by microarry")+
   scale_x_discrete( labels= c("CEG(2014)", "CEG2(2017)", "Housekeeping genes"))
...
myclinicaldata <- rownames_to_column(myclinicaldata, var = "case_name")
...
df <- select(myclinicaldata, c("case_name",clivar)) %>%  
  full_join(AGR2_cn) %>%
  full_join((AGR2_exp)) %>% 
  mutate(AGR2_status= case_when((exp>2 | cn>1) ~ "High", 
                                (exp<0 | cn>0)~ "Low",
                                TRUE~ "unknown")   
         )         
...
p <- autoplot(KM0,xlab="time(month)", ylab="Survival rate",conf.int =F) 
p
...
p <- autoplot(KM0,xlab="time(month)", ylab="Survival rate",conf.int =F) 
p
survdiff(Surv(OS_MONTHS,OS_STATUS_binary) ~ SRGN_status, data = data2)
